...
if [[ $serverc == 1 ]];then
	...
	echo ""
	bash /usr/local/simplessr/server.sh
fi
...
if [[ $serverc == 4 ]];then
	bash /usr/local/simplessr/ss/tail.sh
fi
...
# Default-Stop:         0 1 6
# Short-Description: simplessr
# Description: simplessr
### END INIT INFO
iptables-restore < /etc/iptables.up.rules
bash /usr/local/simplessr/ss/logrun.sh
EOF
    	...
    	cd /etc/init.d
    	update-rc.d simplessr defaults 95
	fi
